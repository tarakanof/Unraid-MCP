...
> `Mcp-Session-Id` to keep pinned to one backend. A pre-2026 client's
> `initialize` is still answered — just without a session. A reverse proxy or
> load balancer can round-robin across replicas freely, and restarting the
> container between two client requests is safe — nothing carries over between
> requests except the read-only Unraid API connection each process opens for
> itself. One legacy wrinkle: a pre-2026 client's standalone `GET /mcp`
> notification stream is accepted but stays silent forever (this server sends
